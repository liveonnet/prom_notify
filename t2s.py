...
import time
import codecs
from conf_lib import conf
from log_lib import app_log
info, debug, warn, error = app_log.info, app_log.debug, app_log.warning, app_log.error

TTS_FLAG_DATA_END = 2

log_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'log')  # dir of script log file
...
class CMSC(object):
    ...
    def getArgs(self):
        '''获取输入的参数
        '''
#-#        parser = OptionParser(version='%prog v0.111215')
        parser = argparse.ArgumentParser()
        parser.add_argument('--version', action='version', version='%(prog)s 20170221')
#-#        parser.add_option('-l', '--print_only', dest='only_print_result', action='store_true', help='don\'t update db, just print stat result', default=False)
        parser.add_argument('-l', '--print_only', dest='only_print_result', action='store_true', help='don\'t update db, just print stat result')
#-#        parser.add_option('-f', '--from', dest='from_file', help='input file', default=None)
        parser.add_argument('-f', '--from', dest='from_file', help='input file', default=None)
#-#        parser.add_option('-t', '--to', dest='to_file', help='output file', default=None)
...
#-#        ret = self.dl.QTTSInit("appid=4ee9a3da,max_text_size=4096,server_url=dev.voicecloud.cn/index.htm")
    ...
    def process_short(self):
        ...
        if ret.value != 0:
            ...
            return
#-#            info('sess_id = %s', ctypes.string_at(sess_id))
        tmp_text = open(self.from_file).read().encode('utf8')

#-#            print ('sending ...\n%s\n'%(tmp_text,), file=sys.stderr)
        rslt = self.dl.QTTSTextPut(sess_id, ctypes.c_char_p(b''.join((tmp_text, '、'.encode('utf-8')))), ctypes.c_int(len(tmp_text) + len('、'.encode('utf-8'))), None)
        if rslt != 0:
        ...
        if s:
            with open(self.to_file, 'wb') as out_file:
                out_file.write(s)
            if len(s) != total_pcm_size:
                warn('write %s != receive %s', len(s), total_pcm_size)
    ...
    def process(self):
        ...
        sess_param = "voice_name = xiaoyan, text_encoding = utf8, sample_rate = 16000, speed = 60, volume = 100, pitch = 50, rdn = 3"
        size_per_call = 1000
        ...
        while flag_ok is True and tmp_text != '':
            # create session
            sess_id = self.dl.QTTSSessionBegin(ctypes.c_char_p(sess_param), ctypes.byref(ret))
            if ret.value != 0:
                info('QTTSSessionBegin() failed! %d', ret.value)
...
#-#            info('sess_id = %s', ctypes.string_at(sess_id))
#-#            print ('sending ...\n%s\n'%(tmp_text,), file=sys.stderr)
            rslt = self.dl.QTTSTextPut(sess_id, ctypes.c_char_p(''.join((tmp_text, '、'.encode('utf-8')))), ctypes.c_int(len(tmp_text)), None)
            if rslt != 0:
                warn('QTTSTextPut() failed! %s', rslt)
                flag_ok = False
                break
...
#-#                    debug('got %s', volLen.value)
            ...
            if s:
                ...
                if len(s) != total_pcm_size:
                    warn('write %s != receive %s', len(s), total_pcm_size)
            rslt = self.dl.QTTSSessionEnd(sess_id, "success")
            print(file=sys.stderr)
            if rslt != 0:
                warn('QTTSSessionEnd() failed! %s', rslt)
